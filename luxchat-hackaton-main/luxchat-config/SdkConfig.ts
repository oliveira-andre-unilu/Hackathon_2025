...
export const DEFAULTS: DeepReadonly<IConfigOptions> = {
    ...
    room_directory: {
        servers: ["test"],
        services: {"test": "test.lu"}
    },
    alias_list: {"test": "test.lu"}
};
...
// Centralized list of valid server names for easier maintenance
const SERVER_NAMES: ServerName[] = [
  "Dev", "CTIE", "PRONOSTIX", "U-01", "U-02", "LHC UAT", "LHC", "[TEST] LU-CIX UAT", "Mixvoip", "HACKATON", "LOCAL", 
  "Luxembourg Online", "ION – Mbox", "ETAT", "LU-CIX UAT", "LU-CIX", "DEV", "EDUCATION Server", "MUNICIPAL SECTOR Server",
  "LoL UAT", "Mixvoip UAT", "POLICE Server", "POLICE/ADA Server", "DEFAULT Server", "U-GUESTS Server", "LAB", "Education", "GUESTS Server"
];

function isServerName(name: string): name is ServerName {
  return SERVER_NAMES.includes(name as ServerName);
}

export function getServices(): Record<ServerUrl, ServerName> {
  const services = SdkConfig.getObject("room_directory")!.get("services");
  if (!services || typeof services !== 'object') {
    return {};
  }

  return Object.entries(services).reduce((acc, [key, value]) => {
    if (typeof key === 'string' && typeof value === 'string' && isServerName(value)) {
      acc[key] = value;
    }
    return acc;
  }, {} as Record<ServerUrl, ServerName>);
}

export function serverUrlToServerName(serverUrl: any, fallbackName?: any): ServerName {
    return getServices()[serverUrl] ?? fallbackName ?? "Unknown Server";
}
